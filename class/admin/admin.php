...
class admin {
    ...
    function upgrade($old_version) {
        ...
        if(version_compare($old_version,'4.8','<')) {
    ...
    function head($title='') {
        ...
        $headhtml.='<script>layui.config({base: \''.template_url().'static/\',version:\'4.8\'}).extend({index: \'lib/index\'}).use([\'index\',\'form\'],function(){});</script>'."\n";
        Return $headhtml;
    }
    function css($check=1) {
        if($check && G('css')){return '';}
        G('css',1);
        Return '<link rel="stylesheet" href="'.template_url().'static/admin.css?v=4.8" media="all">'."\n";
    }
    function icoNav() {
