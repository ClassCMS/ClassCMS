...
class admin_var {
    ...
    function edit() {
        ...
        foreach($array['config'] as $key=>$config) {
            ...
            $array['config'][$key]['ajax_url']='?do=admin:var:ajax&formid='.$array['id'].'&confighash='.$config['hash'].'&csrf='.C('admin:csrfForm');
        }
        ...
        $array['defaultvalue_form']['ajax_url']='?do=admin:var:ajax&formid='.$array['id'].'&confighash=defaultvalue&csrf='.C('admin:csrfForm');
