        ...
        <div class="layui-card">
            <div class="layui-card-header">
                <div class="layui-row">
                    ...
                    {if $auth.channel_add}<a href="?do=admin:channel:add&classhash={$classinfo.hash}{if $fid}&fid={$fid}{/if}" class="layui-btn layui-btn-sm layui-btn-danger"><i class="layui-icon layui-icon-add-1"></i><b>增加栏目</b></a>{/if}
                    </div>
                </div>
            </div>
...
<table class="layui-table" lay-skin="line"  lay-size1="sm">
         <thead>
          <tr>
            <th>栏目名</th>
            {if $auth.channel_edit}<th class="layui-hide-xs">栏目ID</th>{/if}
            {if $auth.module_config}<th class="layui-hide-xs">模型标识</th>{/if}
        ...
        <tbody id="channel">
            ...
            <tr rel="{$channel.id}" data-name="{$channel.channelname}">
                ...
                <td class="btn">
                    ...
                    {if $auth.channel_edit}<a class="layui-btn layui-btn-sm layui-btn-primary" href="?do=admin:channel:edit&id={$channel.id}">修改</a>{/if}
                    {if $auth.channel_del}<a class="layui-btn layui-btn-sm layui-btn-primary channeldel">删除</a>{/if}
                </td>
            </tr>
            {/loop}
        </tbody>
