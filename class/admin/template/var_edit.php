                ...
                <div class="layui-tab-item layui-show">
                 ...
                 <div class="layui-tab-item">
                          <div class="layui-form-item">
                            ...
                            <div class="layui-form-mid">当增加新栏目时,变量的默认值.请先修改"变量配置",保存后再设置默认值</div>
                            </div>
                          </div>
                          <div class="layui-form-item">
                            ...
                            <div class="layui-form-mid">将模型下属栏目变量[{$hash}]全部重置为默认值,请谨慎操作!!!</div>
                            </div>
                          </div>
                 </div>
                 <div class="layui-tab-item">
                          <table class="layui-table">
                            ...
                            <blockquote class="layui-elem-quote layui-text">
                                如角色拥有"模型管理-变量管理"权限,则不受此页面权限限制,拥有全部权限
                            </blockquote>
                 </div>
            </div>
    </div>
...
<script>layui.use(['index'],function(){
    layui.form.on('submit(form-submit)', function(data){
        ...
        layui.admin.req({type:'post',url:"?do=admin:var:editPost",data:data.field,async:true,beforeSend:function(){
    ...
    layui.$('.var_auth_list').each(function(){
    ...
    layui.form.on('checkbox(checkone)', function(data){
      var someone_checked=false;
      layui.$(data.elem).parents('.var_auth_list').find('input[type=checkbox]').each(function(){
            if (layui.$(this).prop("checked"))
            {
                someone_checked=true;
            }
      ...
      if (someone_checked)
      {
          layui.$(data.elem).parents('.var_auth_list').prev().find('input[type=checkbox]').prop("checked", true);
      }else{
          layui.$(data.elem).parents('.var_auth_list').prev().find('input[type=checkbox]').prop("checked", false);
      }
      layui.form.render();
    });
